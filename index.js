...
async function startWhatsApp() {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (msg.message.buttonsResponseMessage) {
      ...
      if (buttonId === 'edit_message') {
        await sock.sendMessage(remoteJid, { text: "Please send the new text you'd like to use for editing." });
        return;
      }
    }
    ...
    try {
      ...
      await sock.sendMessage(remoteJid, {
        ...
        buttons: [
          {
            buttonId: 'edit_message',
            buttonText: { displayText: 'Edit' },
            type: 1,
          },
        ],
        headerType: 1,
